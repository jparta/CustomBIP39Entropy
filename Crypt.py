...
def generate_words(customizer_phrase: str, mnemo: Mnemonic, mnemonic_word_count: int):
    ...
    first_letters_in_beginning = list(customizer_phrase.lower())
    randomized_bits_count, byte_count_without_checksum = get_bitcounts(mnemonic_word_count)
    ...
    fixed_words = []
    fixed_words_indices = []
    words_by_first_letter = {b: [word for word in wordlist if word[0] == b] for b in first_letters_in_beginning}
    ...
    customized_beginning_bits = ''.join(word_index_to_bits(i) for i in fixed_words_indices)
...
def main():
    ...
    recovery_phrases = [generate_words(customizer_phrase, mnemo, mnemonic_word_count) for _ in range(number_of_phrases)]
